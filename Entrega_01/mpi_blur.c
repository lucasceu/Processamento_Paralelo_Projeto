...
int main(int argc, char *argv[]) {
    ...
    if (my_rank == 0) {
        ...
        double gflops   = (2.0 * width * height * k_size * k_size * iterations)
                          / (t_total * 1e9);
        ...
        if (csv) {
            ...
            if (ftell(csv) == 0)
                fprintf(csv, "Imagem,Kernel,Iteracoes,Processos,"
                             ...
                             "GFLOPS,Delta_Max,Corretude\n");
            fprintf(csv, "%s,%d,%d,%d,%.6f,%.6f,%.6f,%.4f,%.2e,%s\n",
                    filename, k_size, iterations, nprocs,
                    t_total, t_comp, t_comm, gflops, (double)delta_max, status);
            fclose(csv);
        }
